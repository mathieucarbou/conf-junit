...
class LongTest2 {
    ...
    static void printMe(where = null) { println '[' +Thread.currentThread().name + '] ' + (where ?: new Throwable().stackTrace.with {StackTraceElement[] els -> els.find({StackTraceElement el -> el?.methodName != 'printMe' && el?.className?.endsWith('LongTest2')})?.methodName}) }

    public static void main(String[] args) {
        JUnitCore.main(LongTest2.class.name)
    }
}
